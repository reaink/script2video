...
| **分镜衔接** | 可选启用（需配置 Gemini）：用 Gemini Flash Image 抽取前镜尾帧作为后镜首帧，保证视觉连贯性 |
...
| **Runway** | gen4.5、gen4_turbo（时长 5s / 10s） |
| **MiniMax** | Hailuo-2.3、Hailuo-2.3Fast、Hailuo-02（时长 6s / 10s） |
| **Luma** | ray-2、ray-flash-2（时长 5–9s） |
